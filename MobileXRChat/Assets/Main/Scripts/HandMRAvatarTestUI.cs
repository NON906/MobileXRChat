...
namespace HandDVR
{
    public class HandMRAvatarTestUI : MonoBehaviour
    {
        ...
        private async Task GetMyVRM()
        {
            try
            {
                ...
                //vrmLoader?.SetupFirstPerson(FPSCameras[0]);
        ...
        private void DoCalibration()
        {
            ...
            if (calibrator == null) calibrator = new FinalIKCalibrator(handMRTracker);
            calibrator?.LoadModel(CurrentModel);
            calibrator?.DoCalibration();

            BioIKHandModelObject.SetModelAnimator(CurrentModel.GetComponentInChildren<Animator>());
        }
